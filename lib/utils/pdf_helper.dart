...
createExcel2(var controller) async{
  ...
  var cel6=sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: 0));
  cel6.value = "Delivery Charge";
  cel6.cellStyle = cellStyle;

  var cel7=sheet.cell(CellIndex.indexByColumnRow(columnIndex: 7, rowIndex: 0));
  cel7.value = "Total Cost";
  cel7.cellStyle = cellStyle;

  var cel8=sheet.cell(CellIndex.indexByColumnRow(columnIndex: 8, rowIndex: 0));
  cel8.value = "Status";
  cel8.cellStyle = cellStyle;

  var cel9=sheet.cell(CellIndex.indexByColumnRow(columnIndex: 9, rowIndex: 0));
  cel9.value = "Codes";
  ...
  for (var row = 1; row <= controller.order.length; row++) {
    ...
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: row)).value = countJustDiscountExport(controller.order[row-1].coupon!.discount??"0.00",
        ...
        controller.order[row-1].qty!,role).toString();
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row)).value = controller.order[row-1].deliveryCarge!;
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 7, rowIndex: row)).value = countJustFinalExport(
        controller.order[row-1].coupon!.discount??"0.00",
        controller.order[row-1].product!.customerPrice!,
        controller.order[row-1].product!.wholesalerPrice!,
        controller.order[row-1].qty!,role,controller.order[row-1].deliveryCarge!).toString();
